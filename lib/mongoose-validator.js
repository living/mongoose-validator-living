...
const interpolateMessageWithArgs = (message = '', args = []) => {
  message = message.replace(/{ARGS\[(\d+)\]}/g, (match, submatch) => args[submatch] || '');
  message = message.replace(/{ARGS-LIST}/g, JSON.stringify(args).replace(/^\[/, "").replace(/\]$/, "").replace(/\"/g, "").replace(/\,/g, ", "));
  return message;
};

const createValidator = function createValidator(fn, args, passIfEmpty) {
  return function validator(val) {
    const validatorArgs = [val, args];
...
const validate = function validate(options) {
  ...
  const messageStr = findFirstString(
    ...
    validate.defaultErrorMessages[validatorName],
    'Error'
  )
